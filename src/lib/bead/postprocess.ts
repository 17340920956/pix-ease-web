 ...
 * 
 * 工作原理：
 * 1. 计算每个像素的局部梯度（到8邻居的平均色差）
 * 2. 纯色区（梯度 < 5）：中值滤波强力平滑，消除色斑
 * 3. 过渡区（梯度 5-15）：轻量平滑
 * 4. 边缘区（梯度 > 15）：不做处理，保留细节
 */
function gradientAwareMedianFilter(
  beadMap: Map<string, string>,
  palette: BeadColor[],
  width: number,
  height: number,
  ...
  const hexToRgb = new Map<string, [number, number, number]>();
  for (const c of palette) {
    const upper = c.hex.toUpperCase();
    paletteLab.set(upper, rgbToLab(c.rgb[0], c.rgb[1], c.rgb[2]));
    hexToRgb.set(upper, [c.rgb[0], c.rgb[1], c.rgb[2]]);
  }
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const rgb = hexToRgb.get(hex.toUpperCase());
      if (!rgb) continue;
      ...
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          if (!nHex) continue;
          const nRgb = hexToRgb.get(nHex.toUpperCase());
          if (!nRgb) continue;

          const dr = rgb[0] - nRgb[0];
          const dg = rgb[1] - nRgb[1];
          const db = rgb[2] - nRgb[2];
          sumDiff += Math.sqrt(dr * dr + dg * dg + db * db);
          count++;
        }
      }
      gradients[y * width + x] = count > 0 ? sumDiff / count : 0;
    }
  }
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (bestHex === hex) continue;

      // 纯色区（梯度 < 5）：邻居色占 4/8 就替换
      // 过渡区（梯度 5-15）：邻居色占 6/8 才替换
      const threshold = gradient < 5 ? 4 : 6;
